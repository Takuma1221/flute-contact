      ...
      try {
        ...
        if (response.ok) {
          ...
          setLiveInfo({
            ...
            liveDate: data.liveDate || "2025年10月4日（土）",
            liveTime1: data.liveTime1 || "14:00",
            liveTime2: data.liveTime2 || "",
            cancelPolicy:
              data.cancelPolicy ||
              "お客様都合によるお申込み後のキャンセルおよび返金はお受けしておりません。予めご了承ください。\nなお、当日現金払いを選択されたお客様でご来場いただけなかった場合には、お手数ですが お振込み下さいますようお願いいたします。",
            cancelDeadlineDays: data.cancelDeadlineDays || 0,
          });
        }
      } catch (error) {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                >
                  ...
                  {liveInfo.liveTime1 && (
                    <option
                      value={`${liveInfo.liveDate}-${liveInfo.liveTime1}`}
                    >
                      {liveInfo.liveDate}
                  ...
                  {liveInfo.liveTime2 &&
                    liveInfo.liveTime2.trim() !== "" &&
                    liveInfo.liveTime2 !== liveInfo.liveTime1 && (
                      <option
                        value={`${liveInfo.liveDate}-${liveInfo.liveTime2}`}
                      >
                        {liveInfo.liveDate}
